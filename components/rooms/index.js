...
const Rooms = props => {
  ...
  const onAddrSwitch = addr => dispatch({ type: 'WRITE_GROUP_ADDRESS', addr: toggleAddrVal(addr) })

  return (
    <div>
      <ShowOnlyActiveToggle />
      <RoomList addresses={filterAddresses(addresses)} prefs={prefs} rooms={rooms} onAddrSwitch={onAddrSwitch} />
    </div>
  )
}
