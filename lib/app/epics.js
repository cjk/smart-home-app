...
import { buffer, filter, merge, mergeMap, map, mapTo, skipWhile, startWith, switchMap, tap } from 'rxjs/operators'
...
export const startConnectionTracking = (action$: any, { clientConnection }: Dependencies) => {
  ...
  return action$.pipe(
    ...
    switchMap(() => states$.pipe(map(newState => updateConnectionState(newState)))),
...
export const startConnErrorTracking = (action$: any, { clientConnection }: Dependencies) => {
  ...
  return action$.pipe(
    ...
    switchMap(() =>
      errors$.pipe(
        tap(s => debug(`Connection error occured <${s}>`)),
