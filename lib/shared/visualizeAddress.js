...
function visualizedAddress(props) {
  ...
  switch (addr.func) {
    case 'light': {
      ...
      return (
        <Avatar className={addr.value ? classes.isOn : classes.isOff}>{addr.value ? <IconOn /> : <IconOff />}</Avatar>
      )
    }
    ...
    default: {
      ...
      switch (addr.type) {
        ...
        default: {
          return <Avatar className={addr.value ? classes.isOn : classes.isOff}>?</Avatar>
        }
      }
    }
  }
