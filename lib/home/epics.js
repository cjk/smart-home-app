...
export const sceneActivate = (action$: any, { connectClient }: Dependencies) =>
  action$.ofType('SCENE_ACTIVATE').switchMap(action => {
    const { sceneId } = action;
    const { client } = connectClient();
    client.event.emit('knx/activateScene', sceneId);
    //     return Observable.of({ type: 'SCENE_ACTIVATE_DONE' });
